...
create-pipeline:
	...
	@aws cloudformation create-stack --stack-name ${APP_NAME}-pipeline --template-body file://pipeline.yaml --parameters \
		...
		ParameterKey=ArtifactBucket,ParameterValue=dpp-artifacts-429431162431-eu-west-1-cfn \
		ParameterKey=GitHubOAuthToken,ParameterValue=${OAUTH_TOKEN} \
		ParameterKey=GitHubUser,ParameterValue=cbonnewijn \
		ParameterKey=GitHubRepository,ParameterValue=lambda-workshop \
